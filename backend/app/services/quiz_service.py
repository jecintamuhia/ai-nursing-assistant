...
_GENERATE_PROMPT = """You are an expert nursing educator.
...
Rules:
- Base every question 100% on the content above
- MCQ: 4 options (A-D), correct_answer = letter, add explanation
- True/False: correct_answer = "True" or "False", add explanation
- Short Answer: correct_answer = 2-3 sentence model answer
...
{{
  "questions": [
    {{
      ...
      "correct_answer": "A",
      "explanation": "...",
      "source_hint": "brief phrase from source"
    }}
  ]
}}
For True/False and Short Answer set options to null."""

_GRADE_PROMPT = """Grade this nursing exam short-answer response.
Question: {question}
Model answer: {model}
...
class QuizService:
    ...
    def reload(self) -> None:
        try:
            ...
            print(f"QuizService reload error: {e}")
    ...
    def generate(self, topic: str, num_questions: int,
        ...
        docs = self._vectorstore.similarity_search(topic, k=settings.quiz_retriever_k)
        if not docs:
    ...
    def grade(self, question: dict, user_answer: str) -> dict:
        ...
        if qtype in ("MCQ", "True/False"):
            ...
            return {"correct": ok, "correct_answer": correct, "explanation": expl,
                    "feedback": "Correct!" if ok else f"The answer is {correct}. {expl}"}
        prompt = _GRADE_PROMPT.format(
            question=question.get("question", ""),
            model=correct, student=user_answer)
        raw = self._llm.invoke(prompt).content.strip()
        raw = re.sub(r"^```json\s*|\s*```$", "", raw)
        try:
